...
export default function Page() {
  ...
  const fetchData = async (page = 1, dscode = '') => {
    try {
      const res = await fetch(`/api/withdrawalreport/month/invalid?page=${page}&limit=10&dscode=${dscode}`)
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className=' font-semibold underline'>Invalid Withdrawal</h1>
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 border text-center">{item.invalidresn}</td>
                </tr>
              ))}
            </tbody>
          </table>
